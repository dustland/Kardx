# OpenKards 架构：设计原则和系统结构

[View English Version](./Arch.md)

_本文档提供 OpenKards 游戏的高级架构指导，概述核心设计原则、系统结构和组件交互。有关数据模型的详细信息，请参阅 [Data-CN.md](./Data-CN.md)。_

## 概述

OpenKards 架构旨在创建一个可维护、可测试和可扩展的代码库，能够随着游戏需求的发展而演变。其核心是强调关注点分离、单向数据流和组件之间的事件驱动通信。
...
4. **CardCollection**：所有卡牌区域（`Hand`、`Deck`、`Battlefield`、`DiscardPile`）的基类
   - 负责区域成员关系、所有者分配和 `ZoneType` 更新
   - 卡牌进入或离开区域时触发 `OnCardAdded` 和 `OnCardRemoved`
   - `Battlefield` 等子类管理槽位布局，但成员变更必须通过 `AddCard` / `RemoveCard` 路由
...
1. **MatchManager**：编排整体游戏流程
   ...
   - 暴露主要游戏状态变化的事件
   - 协调玩家操作和 UI 更新
   - 在变更状态前验证部署、移动、攻击和反制操作

2. **CombatRules**：集中式战斗目标选择与攻击资格规则

   - `CanUnitAttack` 强制执行 Blitz、召唤疲劳和每回合一次攻击限制
   - `IsValidAttackTarget` 验证单位目标，包括 Guard、Smokescreen 和前线规则
   - `CanAttackHQ` 在前线清空时验证对总部的攻击
   - `GetValidAttackTargets` 汇总合法目标，供 UI 高亮和 AI 规划使用

3. **StrategyPlanner**：处理 AI 和规划系统
...
1. **CardDragController**：所有卡牌交互的统一拖拽入口
   - 通过 `CardDragCapability` 解析拖拽模式（部署、指令、反制、攻击、移动）
   - 通过 `CardDragTargetFinder` 查找放置目标
   - 通过 `CardDropResolver` 执行操作，调用 `MatchManager` API
...
3. **ViewManager / ViewRegistry**：跟踪卡牌视图实例
   - 为模型卡牌创建和销毁 `CardView` 对象
   - 将视图生命周期与区域成员变更分离
...
5. **通过 CardCollection 路由区域变更**：

   ```csharp
   // 好：Battlefield 槽位变更委托给 CardCollection
   public bool DeployCard(Card card, int slotIndex)
   {
       ...
       if (!cards.Contains(card))
           AddCard(card); // 设置 owner、zone 并触发 OnCardAdded

       return true;
   }

   // 差：在 Battlefield 中直接修改 cards 列表或触发区域事件
   cards.Add(card);
...
1. 用户将战场单位拖向对手槽位或总部
2. `CardDragCapability` 解析 `CardDragMode.BattlefieldAction` 并检查 `CombatRules.CanUnitAttack`
3. `CardDragTargetFinder` 通过 `MatchManager.CanAttack` 或 `CanAttackHQ` 验证放置目标
4. `CardDropResolver` 调用 `MatchManager.InitiateAttack` 或 `InitiateAttackOnHQ`
5. `MatchManager` 应用伤害、消耗行动费用并触发 `OnAttackCompleted`
6. `MatchView` 和战场视图根据事件和 `OnUIUpdateNeeded` 更新

**验证层次**：

- UI 拖拽资格：`CardDragCapability.CanAttack`
- 目标合法性：`CombatRules.IsValidAttackTarget` / `CanAttackHQ`
- 含费用的模型层门禁：`MatchManager.CanAttack` / `CanAttackHQ`
