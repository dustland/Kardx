...
namespace Kardx.UI.Scenes.Battle
{
  public class BattleView : MonoBehaviour
  {
    ...
    [SerializeField] private Transform battlefieldArea;
    ...
    [SerializeField] private float handCurveHeight = 0.5f;
    ...
    private void ArrangeHandCards()
    {
      ...
      for (int i = 0; i < cardCount; i++)
      {
        ...
        float x = Mathf.Lerp(-cardSpacing * (cardCount - 1) * 0.5f, cardSpacing * (cardCount - 1) * 0.5f, t);
    ...
    {
      if (!cardUIElements.TryGetValue(card, out GameObject cardGO)) return;
      if (!dropZones.TryGetValue(position, out BattlefieldDropZone dropZone)) return;
